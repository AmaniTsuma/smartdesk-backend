...
app.get('/api/test', (req, res) => {
  res.json({ message: 'DATABASE BACKEND - MySQL Connected!' });
});
...
// User profile endpoint (for logged in user)
app.get('/api/auth/me', async (req, res) => {
  try {
    // For demo, return admin user - in real app you'd get from JWT token
    const connection = await pool.getConnection();
    const [rows] = await connection.execute(`
      SELECT id, email, first_name, last_name, role, is_active, company, phone, created_at, updated_at
      FROM users WHERE email = 'info@smartdesk.solutions'
    `);
    connection.release();
...
app.get('/api/service-requests/public', async (req, res) => {
  try {
    // Return hardcoded services for now since we have them working
    const adminServices = [
      {
        id: 'service-1',
...
app.listen(PORT, () => {
  console.log(`🚀 DATABASE BACKEND - Running on port ${PORT}`);
